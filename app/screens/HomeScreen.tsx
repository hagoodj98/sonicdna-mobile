...
export default function Index() {
  ...
  useEffect(() => {
    const requestPermissions = async () => {
      ...
      // Keep playback mode active by default so iOS routes audio to speaker.
  ...
  const handleAudioSubmission = async () => {
    try {
      ...
      if (uriToUpload && titleAudioFile) {
        ...
        if (audioDraft) {
          await uploadAudio(uriToUpload, titleAudioFile.name, audioDraft);
        }
      } else {
        Alert.alert(
          "Missing information",
  ...
  useEffect(() => {
    if (!isModalOpen) {
      ...
      setTitleAudioFile({ name: "" }); // Reset the input value for the audio file name when the modal is closed
    }
  }, [isModalOpen]);
    ...
    <View style={{ flex: 1, backgroundColor: "#0B0F1A" }}>
      ...
      {!isModalOpen && (
        <View
          style={{
            alignItems: "center",
            // Reserve space for the floating tab bar plus bottom safe area.
      ...
      {isModalOpen && (
        ...
        >
          ...
          <Input
            value={titleAudioFile.name}
            readOnly={isFormSubmitting}
            onChangeText={(text) => setTitleAudioFile({ name: text })}
